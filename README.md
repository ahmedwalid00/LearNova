...
Preventing unwanted auto-generated files
---------------------------------------

This repository previously experienced unwanted auto-generated files being recreated during development (for example, by editor file-watchers or hot-reload inside containers). To help prevent that without changing project code, the project includes a lightweight monitor script and additional Docker reload exclusions.

- To run the monitor (removes known unwanted files if they appear):

```bash
python3 scripts/monitor_unwanted_files.py
```

- The Docker compose used for local development includes reload exclusions for common folders that should not be watched by Uvicorn's reload. If you run into file recreation again, start the monitor and then start the compose stack (from the `Docker` folder):
...
Notes:
- The monitoring script is a temporary safeguard and can be disabled once editor/IDE settings or development workflows are adjusted.
- The file `celerybeat-schedule` is generated by Celery and is expected to appear when Celery is running.
